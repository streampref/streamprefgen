...
from gen.directory import STREAMPREF_QUERY_DIR, \
    CQL_QUERY_DIR, DATA_DIR, OUT_DIR, write_to_txt, get_env_file
from gen.experiment import SLI, RAN, VAR, ATT, INTEGER, ALG, SEQ_ALG, \
    CQL_ALG, get_attribute_list, get_query_id, get_id, get_data_id
...
def gen_seq_query(experiment_conf, parameter_conf, directory_dict):
    ...
    '''
    filename = directory_dict[STREAMPREF_QUERY_DIR] + os.sep \
        + get_query_id(experiment_conf, parameter_conf) + '.cql'
    query = SEQ_QUERY.format(ran=experiment_conf[RAN],
                             sli=experiment_conf[SLI])
    write_to_txt(filename, query)


def gen_cql_position_queries(parameter_conf, directory_dict):
    '''
    Generate queries to get each position
    '''
    # Generate W_1
    filename = directory_dict[CQL_QUERY_DIR] + os.sep + 'w1.cql'
    write_to_txt(filename, CQL_W1)
    # Get maximum range
    max_range = parameter_conf[RAN][VAR][-1]
    # W_i
    for range_value in range(2, max_range + 1):
        query = CQL_WI.format(prev=range_value - 1)
        filename = directory_dict[CQL_QUERY_DIR] + os.sep + \
...
def gen_cql_w_queries(experiment_conf, parameter_conf, directory_dict):
    '''
    ...
    query = CQL_W.format(att=att_str, ran=experiment_conf[RAN],
                         sli=experiment_conf[SLI])
    filename = directory_dict[CQL_QUERY_DIR] + os.sep + 'w-' \
        + get_id(experiment_conf, parameter_conf,
                 parameter_list=[ATT, RAN, SLI]) + '.cql'
    write_to_txt(filename, query)


def gen_cql_final_query(experiment_conf, parameter_conf, directory_dict):
    '''
    ...
    query = '\nUNION\n'.join(pos_query_list) + ';'
    filename = directory_dict[CQL_QUERY_DIR] + os.sep + 'equiv-' \
        + get_id(experiment_conf, parameter_conf,
                 parameter_list=[RAN, ATT]) + '.cql'
    write_to_txt(filename, query)
...
def gen_all_cql_queries(experiment_list, parameter_conf, directory_dict):
    ...
    '''
    filename = directory_dict[CQL_QUERY_DIR] + os.sep + 'rpos.cql'
    write_to_txt(filename, CQL_RPOS)
    filename = directory_dict[CQL_QUERY_DIR] + os.sep + 'spos.cql'
    write_to_txt(filename, CQL_SPOS)
    for exp_conf in experiment_list:
        if exp_conf[ALG] == CQL_ALG:
            gen_cql_position_queries(parameter_conf, directory_dict)
            gen_cql_w_queries(exp_conf, parameter_conf, directory_dict)
            gen_cql_final_query(exp_conf, parameter_conf, directory_dict)
...
def gen_seq_env(experiment_conf, parameter_conf, directory_dict, output):
    ...
    if output:
        # Get output filename
        out_file = directory_dict[OUT_DIR] + os.sep + experiment_conf[ALG] + \
            os.sep + get_id(experiment_conf, parameter_conf)
...
def gen_cql_env(experiment_conf, parameter_conf, directory_dict, output):
    ...
    filename = directory_dict[CQL_QUERY_DIR] + os.sep + 'p1.cql'
